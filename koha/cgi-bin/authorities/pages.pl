...
if ( param("location")) {
	...
	my $rfid = param("rfid");             # Get the user's name and assign to variable
	my $barcode = param("barcode");     # Get the choice and assign to variable
	my $shelf = param("location");	
...
#	print $shelf;
	...
	my $sth = $dbh->prepare("INSERT INTO rfid values ('$rfid' ,'$barcode', '$shelf' )");
	$sth->execute() or die $DBI::errstr;
	$sth->finish();

	$dbh->do('DELETE FROM get_tag');
}


if (param("urfid") and param("uid")) {
        use CGI qw(:cgi-lib :standard);  # Use CGI modules that let people read data passed from a form

        my $rfid = param("urfid");             # Get the user's name and assign to variable
  
        my $uid = param("uid");     # Get the choice and assign to variable
...
#       print $shelf;
        ...
        my $sth = $dbh->prepare("UPDATE borrowers set rfid='$rfid' where userid='$uid'");
        $sth->execute() or die $DBI::errstr;
        $sth->finish();

        $dbh->do('DELETE FROM get_user_tag');
}
